...
export function FormAtualizacaoBookingEsteiraOito({ params }: FormAtualizacaoBookingEsteiraOitoProps) {
  ...
  const formularioValido = (dados: DemandaAtualizacao) => {
    const erros: Record<string, string> = {}

    if (dados.liminar_habilitada && !dados.prazo_liminar?.trim()) {
      erros.prazo_liminar = 'Prazo da liminar é obrigatório';
    }

    if (dados.liminar_habilitada && !dados.conteudo_liminar?.trim()) {
      erros.conteudo_liminar = 'Conteúdo da liminar é obrigatório';
    }

    if (dados.audiencia_habilitada && !dados.data_audiencia?.trim()) {
      erros.data_audiencia = 'Data e hora da audiência é obrigatória';
    }

    if (dados.sentenca_habilitada && !dados.conteudo_sentenca?.trim()) {
      erros.conteudo_sentenca = 'Conteúdo da sentença é obrigatório';
    }

    if (dados.despacho_habilitado && !dados.despacho_conteudo?.trim()) {
      erros.despacho_conteudo = 'Conteúdo do despacho é obrigatório';
    }

    if (!dados.liminar_habilitada && !dados.audiencia_habilitada && !dados.sentenca_habilitada && !dados.despacho_habilitado) {
      toast.warning({
        title: 'Aviso!',
        description: 'Nenhuma tarefa selecionada, para prosseguir, ao menos uma tarefa deve ser selecionada',
      });
    ...
    if (Object.keys(erros).length > 0) {
      const dicionarioCamposRequired = {
        prazo_liminar: 'Prazo de liminar é obrigatório',
        conteudo_liminar: 'Conteúdo de liminar é obrigatório',
        data_audiencia: 'Data e hora da audiência é obrigatória',
        conteudo_sentenca: 'Conteúdo da sentença é obrigatório',
        despacho_conteudo: 'Conteúdo do despacho é obrigatório',
      };
    ...
    Object.entries(erros).forEach(([campo, mensagem]) => {
      formBag.setError(campo as keyof DemandaAtualizacao, { type: 'required', message: mensagem });
    });
  ...
  const handleHabilitarDespacho = (checked: boolean) => {
    if (checked) {
      ...
      formBag.clearErrors('despacho_conteudo');
    }
  };
  ...
  const handleHabilitarLiminar = (checked: boolean) => {
    ...
    if (!checked) {
      ...
      formBag.setValue('prazo_liminar', '');
    }

    formBag.clearErrors('conteudo_liminar');
    formBag.clearErrors('prazo_liminar');
  };
  ...
  const handleHabilitarSentenca = (checked: boolean) => {
  ...
  const handlePrazoTipoChange = () => {
    ...
    formBag.clearErrors('prazo_tipo');
  };
                      ...
                      {values.liminar_habilitada && (
                        <div className="space-y-3 mt-2">
                          <FieldMessage.Error.Root>
                            ...
                            <FieldMessage.Error.Text visible={!!formBag.formState.errors.prazo_liminar}>
                              {formBag.formState.errors.prazo_liminar?.message}
                          ...
                          <FieldMessage.Error.Root>
                            ...
                            <FieldMessage.Error.Text visible={!!formBag.formState.errors.conteudo_liminar}>
                              {formBag.formState.errors.conteudo_liminar?.message}
                            </FieldMessage.Error.Text>
                          </FieldMessage.Error.Root>
                        </div>
                      )}
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <div className="flex items-center gap-2 ml-auto">
                          <Checkbox 
                            ...
                            onCheckedChange={handleHabilitarSentenca}
                            disabled={values.despacho_habilitado}
                          />
                      ...
                      {values.sentenca_habilitada && (
                        <div className="space-y-3 mt-2">
                          <FieldMessage.Error.Root>
                            ...
                            <FieldMessage.Error.Text visible={!!formBag.formState.errors.conteudo_sentenca}>
                              {formBag.formState.errors.conteudo_sentenca?.message}
                            </FieldMessage.Error.Text>
                          </FieldMessage.Error.Root>
                        </div>
                      )}
                      ...
                      {values.despacho_habilitado && (
                        <div className="space-y-3 mt-2">
                          <FieldMessage.Error.Root>
                            ...
                            <FieldMessage.Error.Text visible={!!formBag.formState.errors.despacho_conteudo}>
                              {formBag.formState.errors.despacho_conteudo?.message}
                            </FieldMessage.Error.Text>
                          </FieldMessage.Error.Root>
                        </div>
                      )}
      ...
      >
        ...
        >
          ...
          <div className="text-sm">
            <p>Atenção! Ao habilitar o despacho, todos os outros campos (Liminar, Audiência e Sentença) serão <strong>limpos e desabilitados</strong>.</p>
