...
export function FormProconBookingEsteiraOito({ params }: FormProconBookingEsteiraOitoProps) {
  ...
  const formatInitialFormValues = useCallback((values: Partial<DemandaProcon>) => {
  ...
  const validaDataReclamacao = (data: Date | null) => {
    if (!data) {
      formBag.trigger('data_reclamacao');
      return null
    }
    ...
    if (data > today) {
      toast.warning({
        title: 'Aviso!',
        description: 'A data de reclamacao não pode ser superior a data atual.',
      });

      formBag.setError('data_reclamacao', {
        message: 'A data de reclamacao não pode ser superior a data atual.',
      });

      return 'A data de reclamacao não pode ser superior a data atual.'
    }
  ...
  const formularioValido = (dados: DemandaProcon) => {
    ...
    const errosDataReclamacao = validaDataReclamacao(dados.data_reclamacao ? new Date(dados.data_reclamacao) : null); 
    const errosDataAudiencia = validaDataAudiencia(dados.data_audiencia ? new Date(dados.data_audiencia) : null);

    if (errosDataReclamacao) {
      erros.data_reclamacao = errosDataReclamacao;
    }
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                ...
                <DetalhesProcon 
                  ...
                  validaDataReclamacao={validaDataReclamacao}
                />
