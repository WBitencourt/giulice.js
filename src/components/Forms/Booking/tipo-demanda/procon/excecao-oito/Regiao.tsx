...
import { Controller, useFormContext, UseFormReturn, useFormState, useWatch } from 'react-hook-form';
...
const RegiaoComponent = ({ formBag }: RegiaoProps) => {  
  ...
  const { setValue, control } = formBag;
  ...
  const handleUfChange = (newValue: string, onChange: (value: string) => void) => {
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select 
                    value={field.value} 
                    onValueChange={(value) => {
                      handleUfChange(value, field.onChange);
                    }}>
                    <SelectTrigger className={errors.uf ? "border-red-500" : "" } disabled>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    <SelectTrigger className={errors.cidade ? "border-red-500" : ""} disabled>
