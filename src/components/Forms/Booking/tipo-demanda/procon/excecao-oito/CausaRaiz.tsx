...
const CausaRaizComponent = ({ formBag }: CausaRaizProps) => {
  ...
  const values = {
    ...
    sub_causa_raiz: useWatch({ control, name: 'sub_causa_raiz' }),
  }

  const dicionarioPalavrasRelacionadaVoos = [
    'Flight',
  ];
                ...
                render={({ field }) => (
                  <div className="flex items-center gap-2">
                    <Select
                      ...
                      onValueChange={field.onChange}
                    >
                      <SelectTrigger ref={field.ref} className={errors.sub_causa_raiz ? 'border-red-500' : ''}>
