...
const DetalhesAcaoComponent = ({ formBag, dadosConsultaProcesso }: DetalhesAcaoProps) => {
  ...
  const handleDataDistribuicaoChange = (selectedDate: Date | undefined, field: ControllerRenderProps<DemandaCadastroJuridico, "data_distribuicao">) => {
    if (!selectedDate) {
      field.onChange('');
      return;
    }
  ...
  const handleDataCitacaoChange = (selectedDate: Date | undefined, field: ControllerRenderProps<DemandaCadastroJuridico, "data_citacao">) => {
    if (!selectedDate) {
      field.onChange('');
      return;
    }
        ...
        <FieldMessage.Error.Root>
          <div className="space-y-2">
            ...
            <Controller
              name="data_audiencia"
              control={control}
              render={({ field: fieldDataAudiencia }) => (
                <Controller
                  name="campos_auditados.data_audiencia"
                  control={control}
                  render={({ field: fieldAuditado }) => (
                    <div className="group flex gap-2 items-center">
                      ...
                      >
                        <DateTimePicker
                          date={fieldDataAudiencia.value ? new Date(fieldDataAudiencia.value) : undefined}
                          onSelect={(selectedDate) => {
                            handleDataAudienciaChange(selectedDate, fieldDataAudiencia);
                          }}
                          disabled
                        />
                      </ValidadorCampoAuditoria>
