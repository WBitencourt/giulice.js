...
export function FormCadastroJuridicoBookingEsteiraOito({ params }: FormCadastroJuridicoBookingEsteiraOitoProps) {
  ...
  const validaDataAudiencia = (data: Date | null) => {
    if (!data) {
      formBag.trigger('data_audiencia');
      return null
    }
    ...
    if (ehRetroativa && ehFinalDeSemana) {
      ...
      return 'A data da audiência não pode ser em um final de semana ou retroativa.'
    }
    ...
    if (ehFinalDeSemana) {
      ...
      return 'A data da audiência não pode ser em um final de semana.'
    }
    ...
    if (ehRetroativa) {
      ...
      return 'A data da audiência não pode ser retroativa.'
    }

    // Se não tiver erro, atualiza o campo para que o erro seja removido
    // Isso é necessário para que nao renderize todos os componentes do form principal (Evitar clearErrors)
    formBag.trigger('data_audiencia');

    return null
  }

  const validaDataDistribuicao = (data: Date | null) => {
    if (!data) {
      toast.warning({
        title: 'Aviso!',
        description: 'A data de distribuição é obrigatória.',
      });

      return 'A data de distribuição é obrigatória.'
    }
    ...
    if (data >= today) {
      ...
      return 'A data de distribuição não pode ser igual ou superior a data atual.'
    }

    // Se não tiver erro, atualiza o campo para que o erro seja removido
    // Isso é necessário para que nao renderize todos os componentes do form principal (Evitar clearErrors)
    formBag.trigger('data_distribuicao');

    return null
  }

  const validaDataCitacao = (data: Date | null) => {
    if (!data) {
      formBag.trigger('data_citacao');
      return null
    }
    ...
    if (data >= today) {
      toast.warning({
        title: 'Aviso!',
        description: 'A data de citacao não pode ser igual ou superior a data atual.',
      });

      return 'A data de citacao não pode ser igual ou superior a data atual.'
    }
  ...
  const formularioValido = (dados: DemandaCadastroJuridico) => {
    ...
    const errosDataDistribuicao = validaDataDistribuicao(dados.data_distribuicao ? new Date(dados.data_distribuicao) : null);
    const errosDataCitacao = validaDataCitacao(dados.data_citacao ? new Date(dados.data_citacao) : null);
    const errosDataAudiencia = validaDataAudiencia(dados.data_audiencia ? new Date(dados.data_audiencia) : null);

    if (errosDataDistribuicao) {
      erros.data_distribuicao = errosDataDistribuicao;
    }
    
    if (errosDataCitacao) {
      erros.data_citacao = errosDataCitacao;
    }

    if (errosDataAudiencia) {
      erros.data_audiencia = errosDataAudiencia;
    }
    ...
    if (dados.causa_raiz && dados.causa_raiz.includes('Flight') && !dados.sub_causa_raiz) {
      erros.sub_causa_raiz = 'Subcausa raiz é obrigatória';
    }
    ...
    if (Object.keys(erros).length > 0) {
      const dicionarioCamposRequired = {
        ...
        data_citacao: 'Data de citacao',
        data_audiencia: 'Data de audiencia',
        resumo_processo: 'Resumo do processo',
        valor_causa: 'Valor da causa',
        causa_raiz: 'Causa raiz',
        sub_causa_raiz: 'Subcausa raiz',
  ...
  const onSubmitDemanda = async (dados: DemandaCadastroJuridico) => {
    try {
      formBag.clearErrors();
  ...
  const onSubmit = async (dados: DemandaCadastroJuridico) => {
    try {
      ...
      if (dados.desdobramento) {
        await onSubmitDesdobramento(dados);
      } else {
        await onSubmitDemanda(dados);
      }
    } catch (error: any) {
      toast.error({
        title: 'Falha ao enviar o formulário',
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                ...
                <DetalhesProcesso 
                  ...
                  isSavingDesdobramento={isSavingForm}
                  onSalvarDesdobramento={salvarDesdobramento}
                />

                <DetalhesAcao 
                  formBag={formBag}
                  dadosConsultaProcesso={dadosConsultaProcesso}
                  validaDataAudiencia={validaDataAudiencia}
                  validaDataDistribuicao={validaDataDistribuicao}
                  validaDataCitacao={validaDataCitacao}
                />
                ...
                <div className="flex justify-center space-x-2">
                  ...
                  <Button 
                    type="submit" 
                    disabled={formBag.formState.isSubmitting || Object.keys(formBag.formState.errors).length > 0 || formBag.watch('desdobramento')}
                  >
